/**
 * Sujet numérique : Concours Général des Métiers Bac Pro MELEC — session 2021,
 * « Espace d'innovation partagé (EIP) du lycée Mireille Grenet de Compiègne ».
 *
 * Copie conforme du dossier sujet (32 pages, 72 questions, 4 parties) et du dossier
 * technique et ressources (DTR, 52 pages). Les énoncés sont ceux du papier ; les valeurs
 * de correction sont celles du corrigé officiel (les écarts relevés sont signalés en
 * commentaire « Corrigé officiel : … »).
 *
 * Images : `public/tp/cgm2021/` — pages `sujet-01…32.jpg`, `dtr-01…52.jpg` et recadrages
 * des figures (schémas Q13 / Q58 / Q67, plan 3D Q39-Q40, cavaliers Q57…).
 * Positions des bornes et des bulles : en % de l'image recadrée.
 */
import type {
  CelluleSaisie, DtrPage, QCocher, QSchema, SujetNumerique, SujetPartie, SujetQuestion,
} from '@/lib/sujet/types';

const IMG = '/tp/cgm2021';
...
const DTR_TITRES: [string, string][] = [
  ['Page de garde du DTR', 'Habilitations / Sécurité'],
  ['Organigramme et habilitations', 'Habilitations / Sécurité'],
...
const DTR: DtrPage[] = DTR_TITRES.map(([titre, section], i) => ({
  num: i + 1, src: `${IMG}/dtr-${pad(i + 1)}.jpg`, titre, section,
}));

const SUJET_TITRES: [string, string][] = [
  ['Page de garde', 'Présentation'],
  ['Le lycée Mireille Grenet', 'Présentation'],
  ['L’espace d’innovation partagé', 'Présentation'],
...
const PAGES_SUJET: DtrPage[] = SUJET_TITRES.map(([titre, section], i) => ({
  num: i + 1, src: `${IMG}/sujet-${pad(i + 1)}.jpg`, titre, section,
}));
...
const PARTIES: SujetPartie[] = [
  {
    ...
    situation:
      ...
      + '(Dossier technique et ressources pages 2/52 à 6/52.)',
    dtrPages: [2, 3, 4, 5, 6],
  },
  {
    num: 2,
    ...
    situation:
      ...
      + '(Dossier technique et ressources pages 7/52 à 11/52.)',
  ...
  },
  {
    ...
    situation:
      ...
      + '(Dossier technique pages 12/52 à 41/52.)',
  ...
  },
  {
    ...
    situation:
      ...
      + '(Dossier technique pages 42/52 à 52/52.)',
...
const QUESTIONS: SujetQuestion[] = [
  ...
  },
  {
    ...
    // condamnation, identification, VAT ; la correction suit le corrigé du concours).
    rangs: [4, 2, 3, 1],
  ...
  },
  {
    ...
    traits: Q13_TRAITS,
    platineTpId: 'cgm2021-q13-cumulus',
  ...
  },
  {
    ...
    traits: Q58_TRAITS,
    platineTpId: 'cgm2021-q58-myhome',
...
export const CGM2021_EIP: SujetNumerique = {
  id: 'cgm2021-eip',
  titre: 'CGM Bac Pro MELEC 2021 — EIP du lycée Mireille Grenet',
  sousTitre: 'Concours Général des Métiers · Métiers de l’électricité et de ses environnements connectés · Session 2021 · Lycée Mireille Grenet de Compiègne (Oise)',
  diploma: 'bacpro',
  dureeMin: 300,
  consignes: [
    'Durée : 5 heures.',
    ...
    'Le sujet se compose de 32 pages, numérotées de 1/32 à 32/32. L’épreuve comporte 2 dossiers : un DOSSIER SUJET et un DOSSIER TECHNIQUE ET RESSOURCES.',
