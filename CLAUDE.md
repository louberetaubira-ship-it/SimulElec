...
## Sujets numériques (copies conformes de sujets d'examen)
- Contrat : `src/lib/sujet/types.ts` ; moteur générique dans `src/lib/sujet/` ; un sujet = ses données dans `src/lib/data/sujets/` — un fichier (`eip.ts`, `scierie.ts`) ou, pour un gros dossier, un dossier (`chevrerie/` : `commun.ts` + `partie-a.ts`…`partie-g.ts` + `index.ts` qui assemble et numérote). Chaque question porte son outil, son corrigé, 3 aides graduées, ses erreurs typiques et une `formuleSpec` pour tout calcul.
- Anonymat : rien de ce qui est servi au navigateur ne trahit l'origine (commune, entreprise, code du dossier…). Les termes interdits ne sont écrits nulle part en clair : empreintes SHA-256 dans `scripts/anonymisation/empreintes.json`, pages rendues et masquées par `scripts/anonymisation/anonymiser-pages.py @scripts/anonymisation/<sujet>.args` (contrôle OCR), noms neutres pour les acteurs cités (« DURAND », « SYLVA »).
- Un schéma à compléter (`type: 'schema'`) se corrige en traits ET en câblage réel sur un TP platine caché (`hidden: true`, `src/lib/data/tps/<sujet>-<question>.ts`) doté d'un modèle de fonctionnement dans `src/components/sujet/CablageReelModeles.ts` (`MODELES`). Les appareils qui manquent sont créés au catalogue (photo de `public/lib` ou dessin dans `svg.tsx`).
- Images de corrigé dans `private/corriges/<sujet>/` (route gardée `/api/sujet/image`), jamais dans `public/`.
- `npx tsx scripts/audit-sujet.ts` avant toute livraison (numérotation, images, anonymat, corrigé côté serveur, aides, TP platine) ; `--meta` régénère `src/lib/sujet/meta.ts` ; `scripts/test-sujet-correction.ts` couvre le moteur.
