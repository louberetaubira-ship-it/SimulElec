...
import { LIBELLE_OUTIL, texteAttendu } from '@/lib/sujet/format';
  ...
  return (
    <div className="mx-auto max-w-[900px] p-3 sm:p-4" key={q.num} data-question={q.num}>
      ...
      <div className="mb-2 flex flex-wrap items-center gap-1.5 text-[11.5px] text-muted">
        ...
        {q.dtr.map(n => (
          <button key={n} type="button" onClick={() => ouvrirPage('dtr', n)} className="rounded-md border border-line bg-surface px-1.5 py-0.5 font-semibold text-ink hover:border-accent">Page DTR {n}</button>
        ))}
      ...
      <h2 className="mb-1 flex items-baseline gap-2 text-[16px] font-bold leading-snug">
        <span className="flex-none font-title text-[20px] text-accent">Q{q.num}</span>
...
function imageDansOutil(q: SujetQuestion): boolean {
  ...
  if (q.type === 'bulles') return q.image.src === q.plan.src;
  return false;
}
